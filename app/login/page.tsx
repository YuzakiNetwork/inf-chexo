...
export default function Login() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const authResult = await withTimeout(client.auth.signInWithPassword({ email: email.trim(), password })) as SupabaseResult<{ user: { id: string } | null; session: unknown }>;
      const { data, error: authError } = authResult;
      if (authError || !data.user) { setError(authError?.message || 'Login gagal. Periksa email dan password.'); return; }

      const profileResult = await withTimeout(client.from('profiles').select('role, full_name').eq('id', data.user.id).maybeSingle()) as SupabaseResult<{ role: string | null; full_name: string | null } | null>;
      const { data: profile, error: profileError } = profileResult;
      if (profileError) { console.error('[CHEXO] profile lookup failed', profileError); setError(`Profil tidak dapat diverifikasi: ${profileError.message}`); return; }
      if (!profile?.role) { setError('Akun berhasil login, tetapi profil CHEXO belum memiliki role. Hubungi administrator.'); return; }

      const role = profile.role as UserRole;
      const portal = portalForRole(role);
      if (portal === '/login') { setError('Role akun tidak dikenali oleh CHEXO. Hubungi administrator.'); return; }
